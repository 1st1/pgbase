...
cdef inline pgbase_as_pg_string_and_size(
...
cdef pgbase_text_encode(CodecContext settings, WriteBuffer buf, obj):
    cdef:
        char *str
        ssize_t size
...
cdef inline pgbase_decode_pg_string(
        CodecContext settings, const char* data, ssize_t len):
...
cdef pgbase_text_decode(CodecContext settings, FastReadBuffer buf):
    cdef ssize_t buf_len = buf.len
    return pgbase_decode_pg_string(settings, buf.read_all(), buf_len)
