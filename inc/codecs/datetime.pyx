...
cdef pgbase_date_encode(
        CodecContext settings, WriteBuffer buf, obj):
    cdef:
...
cdef pgbase_timestamp_encode_tuple(
        CodecContext settings, WriteBuffer buf, obj):
    cdef:
...
cdef pgbase_timestamp_decode(CodecContext settings, FastReadBuffer buf):
    cdef:
        int64_t seconds = 0
...
cdef pgbase_timestamp_decode_tuple(
        CodecContext settings, FastReadBuffer buf):
    cdef:
...
cdef pgbase_timestamptz_encode(
        CodecContext settings, WriteBuffer buf, obj):
    if not cpython.datetime.PyDateTime_Check(obj):
        if cpython.datetime.PyDate_Check(obj):
            obj = datetime.datetime(obj.year, obj.month, obj.day,
...
cdef pgbase_timestamptz_decode(
        CodecContext settings, FastReadBuffer buf):
    cdef:
        int64_t seconds = 0
        uint32_t microseconds = 0
...
cdef pgbase_time_decode(CodecContext settings, FastReadBuffer buf):
    cdef:
        int64_t seconds = 0
        uint32_t microseconds = 0
...
cdef pgbase_timetz_encode_tuple(
        CodecContext settings, WriteBuffer buf, obj):
...
cdef pgbase_timetz_decode(CodecContext settings, FastReadBuffer buf):
    time = pgbase_time_decode(settings, buf)
...
cdef pgbase_timetz_decode_tuple(
        CodecContext settings, FastReadBuffer buf):
    cdef:
...
cdef pgbase_interval_encode_tuple(
        CodecContext settings, WriteBuffer buf, tuple obj):
    cdef:
        int32_t months
        int32_t days
        int64_t microseconds
...
cdef pgbase_interval_decode(CodecContext settings, FastReadBuffer buf):
    cdef:
        int32_t days
        int32_t months
        int32_t years
...
cdef pgbase_interval_decode_tuple(
        CodecContext settings, FastReadBuffer buf):
